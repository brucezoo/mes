...
$page = 1;
$pageSize = 500;
$dataList = $Drawing->getData('*', array(), array(), true, $page, $pageSize);
while ($dataList) {
    foreach ($dataList as $k => $data) {
        ...
        $rd_res = $Drawing->pdo->fixInsertSql('ruis_drawing', $insertKeyVal_drawing);
        $length_res = $Drawing->pdo->fixInsertSql('ruis_drawing_attribute', $insertKeyVal_length);
        $width_res = $Drawing->pdo->fixInsertSql('ruis_drawing_attribute', $insertKeyVal_width);
        $height_res = $Drawing->pdo->fixInsertSql('ruis_drawing_attribute', $insertKeyVal_height);
        $length2_res = $Drawing->pdo->fixInsertSql('ruis_drawing_attribute', $insertKeyVal_length2);
        $width2_res = $Drawing->pdo->fixInsertSql('ruis_drawing_attribute', $insertKeyVal_width2);
        $height2_res = $Drawing->pdo->fixInsertSql('ruis_drawing_attribute', $insertKeyVal_height2);
        $model_res = $Drawing->pdo->fixInsertSql('ruis_drawing_attribute', $insertKeyVal_model);
        $sqlArr = array(
            $rd_res['sql'],
            $length_res['sql'],
            $width_res['sql'],
        ...
        $paramArr = array(
            ...
            $width_res['paramArr'],
            $height_res['paramArr'],
            $length2_res['paramArr'],
            $width2_res['paramArr'],
            $height2_res['paramArr'],
            $model_res['paramArr']
        );

        $rda_res = $Drawing->pdo->queryWithTransaction($sqlArr, $paramArr);
        echo 'drawing_id:' . $data['id'] . ', res:' . $rda_res . "\n";
    }
    $page++;
    $dataList = $Drawing->getData('*', array(), array(), true, $page, $pageSize);
}
