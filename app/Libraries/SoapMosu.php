...
class SoapMosu
{
    ...
    {
        self::checkFile(config('app.mosu_webservice.wsdl_path'));
        $option = [
            'exceptions' => true,
            'trace' => 1,
            'encoding' => 'UTF-8',
            //'login' => config('app.mosu_webservice.username'),
            //'password' => config('app.mosu_webservice.password'),
            'default_socket_timeout' => 600,
        ];
        self::$soap = new \SoapClient(config('app.mosu_webservice.wsdl_path'), $option);
    }
    ...
    public static function getWSDLFile($wsdl_path)
    {
        $auth = sprintf('Authorization: Basic %s',
            base64_encode(config('app.mosu_webservice.username') . ':' . config('app.mosu_webservice.password'))); // 加入这句
        $opt = array(
            'http' => array(
                'method' => 'GET',
        ...
        try {
            $wsdl_str = file_get_contents(self::getHost() . config('app.mosu_webservice.wsdl_url'), false, $context);
        } catch (\Exception $e) {
            TEA('2453');
        }
    ...
    {
        return config('app.mosu_webservice.intranet_host');

//        if (self::isIntranet()) {
//            return config('app.mosu_webservice.intranet_host');
//        } else {
//            return config('app.mosu_webservice.external_host');
//        }
    }
     ...
     * @throws \App\Exceptions\ApiException
     */
    public static function doRequest($content, $cate_code = 'MOSU_FORM', $code = 'PUR_MOSU_FORM_IMP',$header)
    {
        self::getInstance();
        ...
        // 添加记录
        $recordID = self::makeRecord($cate_code . '#' . $code, $header['head'], $data, []);
        if ($recordID) session(['sap_api_record_id' => $recordID]);

        // 执行请求
        try {
            $response = self::$soap->ZfmService($data);
    ...
    public static function getHeader($cate_code, $code)
    {
        return [
            ...
            'IF_CODE' => $code,
            'USER_NAME' => config('app.mosu_webservice.username'),
            'PASSWORD' => config('app.mosu_webservice.password'),
            'BATCH_NUM' => 1,
            'SEG_NUM' => 1,
            'TOTAL_SEG_COUNT' => 1
