...
/**
 * Sap PO 策略 根据Sap请求参数中的工艺路线找到每个工序对应的唯一工作中心
 * Class SapProductOrderStrategy
 * @package App\Libraries\ProductOrderStrategy
 * @author Bruce.Chu
 */
class SapProductOrderStrategy extends Base implements ProductOrderStrategy
{
    private $input;
    private $workBenchOperationAbilityModel;
    private $procedureModel;
    private static $sapProductOrder;
     ...
     * 单例模式 已废弃
     * @param $input
     * @return SapProductOrderStrategy
     */
    public static function getInstance($input)
    {
        if(!self::$sapProductOrder) self::$sapProductOrder=new self($input);
        return self::$sapProductOrder;
    }
    ...
    public function getWorkCenterInfo()
    {
        //前端传参
        $input=$this->input;
        //po绑定的工艺路线上的工序 sap推送过来的数据
        $routing_info=$this->getFieldValueById($input['production_order_id'],'confirm_number',config('alias.rpo'));
        //工序与工作中心配置 缺失
        if(empty($routing_info)) TEA('2485');
        $routing_info=json_decode($routing_info,true);
        //声明结果返回数组
        $results=[];
        //时间段中实际共有多少天
        ...
        //遍历工序集合 每个工序对应为一个工作中心
        foreach ($routing_info as $value) {
            //验证sap请求工序字段是否配置 即根据sap的工序顺序 是否能找到可用的工序编码 同步po时已处理
            if(!isset($value['operation_code'])) continue;
            //工作中心信息 包括厂 车间 工作中心名称
            $workcenter_info = DB::table(config('alias.rwc') . ' as rwc')
            ...
            //工序也存入结果返回数组
            $workcenter_info->operation_id=$this->getIdByFieldValue('code',$value['operation_code'],config('alias.rio'));
            $workcenter_info->operation_code=$value['operation_code'];
