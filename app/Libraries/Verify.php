...
class Verify
{
    protected $config =	array(
        ...
        'reset'     =>  true,           // 验证成功后是否重置,即验证成功后是否要删除对应的验证码session,默认是true的
    );

    private $_image   = NULL;     // 验证码图纸实例
    ...
    public function check($code, $id = '') {
        ...
        // session 过期  verify_time是验证码生成时间
    ...
    public function entry($id = '') {
        ...
        //判断一下是保存还是直接返回图纸
        if($this->save){
            $d='./captcha/'.date('Y-m-d'); // 接收文件目录
            if (! file_exists ($d)) @mkdir ($d, 0777, true );
            $path=$d.'/'.md5(uniqid(mt_rand(), true)).'.png';
