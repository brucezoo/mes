...
use App\Http\Models\Subarea;//引入分区处理类
...
class SubareaController extends Controller
{
    ...
    {
      parent::__construct();
      if(empty($this->model)) $this->model=new Subarea();
    }
    ...
    public function checkFormFields(&$input)
    {
        ...
        trim_strings($input);
        //code
        if(empty($input['code'])) TEA('8102','code');
        if(!preg_match('/^[A-Z]{1,10}+$/',$input['code'])) TEA('8101','code');
        //name
        if(empty($input['name'])) TEA('8100','name');
        //employee_id
        if(!isset($input['employee_id']) || !is_numeric($input['employee_id']))  TEA('730','employee_id');
      
        if(!isset($input['remark'])) TEA('732','remark');
        if( mb_strlen($input['remark'])>500)  TEA('8111','remark');
    }
    ...
    public function  select(Request $request)
    {
    	//过滤,判断并提取所有的参数
        $input=$request->all();
      	$obj_list=$this->model->getSubareaList($input);
    ...
    public function store(Request  $request)
    {
            ...
            $response=get_api_response('200');
            $response['results']=['subarea_id'=>$insert_id];
            return  response()->json($response);
    }
    ...
    public  function update(Request $request)
    {
        ...
        //拼接返回值
        $response=get_api_response('200');
        $response['results']=['subarea_id'=>$input['id']];
        return  response()->json($response);
    }
