...
use App\Http\Models\WorkMachine;
...
class WorkMachineController extends Controller{
    ...
    {
        parent::__construct();
        if(!$this->model) $this->model = new WorkMachine();
    }
...
//region 查
    ...
    public function pageIndex(Request $request){
        ...
        $obj_list = $this->model->getWorkMachineListByPage($input);
    ...
    public function select(Request $request){
        $input = $request->all();
        $obj = $this->model->getWorkMachineList($input);
        return response()->json(get_success_api_response($obj));
    }
...
//region 删
    ...
    public function delete(Request $request){
        $id = $request->input($this->model->apiPrimaryKey);
        if(empty($id)) TEA('700',$this->model->apiPrimaryKey);
        $obj = $this->model->getRecordById($id,['status']);
        if($obj->status) TEA('1102');
