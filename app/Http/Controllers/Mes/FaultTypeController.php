...
use App\Http\Models\FaultType;//引入仓位处理类
...
class FaultTypeController extends Controller
{
    ...
    {
      parent::__construct();
      if(empty($this->model)) $this->model=new FaultType();
    }
    ...
    public function checkFormFields(&$input)
    {
        ...
        trim_strings($input);
        //code
        if(empty($input['code'])) TEA('9202','code');
        if(!preg_match('/^[A-Z]{1,10}+$/',$input['code'])) TEA('9201','code');

        //name
        if(empty($input['name'])) TEA('9200','name');
      
        if(!isset($input['remark'])) TEA('732','remark');
        if( mb_strlen($input['remark'])>500)  TEA('9210','remark');
