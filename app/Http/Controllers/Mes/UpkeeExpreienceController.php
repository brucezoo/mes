...
use App\Http\Models\UpkeeExpreience;//引入仓位处理类


class UpkeeExpreienceController extends Controller
{
	/**
     * 构造方法初始化操作类
     */
    ...
    {
      parent::__construct();
      if(empty($this->model)) $this->model=new UpkeeExpreience();
    }
    ...
    public function checkFormFields(&$input)
    {
        //过滤
        trim_strings($input);
        // if(!isset($input['upkee_part'])) TEA('732','upkee_part');
        // if( mb_strlen($input['upkee_part'])>500)  TEA('8210','upkee_part');
        // if(!isset($input['upkee_require'])) TEA('732','upkee_require');
        // if( mb_strlen($input['upkee_require'])>500)  TEA('8210','upkee_require');
    ...
    public function store(Request  $request)
    {
        ...
        $response['results']=['require_id'=>$insert_id];
    ...
    public function  show(Request $request)
    {
        ...
        //呼叫M层进行处理
        $response=get_api_response('200');
        $response['results']=$this->model->getOne($id);
        return  response()->json($response);
    }
