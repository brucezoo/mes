...
//region  增
    ...
    public function add($input,$category_id)
    {
        ...
        $input['key'] = $encodingDao->useEncoding(3,$input['key']);
        $data=[
            'key' => $input['key'],
            'name' => $input['name'],
            'label' => $input['label'],
...
//region  查
     ...
     * @reviser   sam.shan <sam.shan@ruis-ims.cn>
     */
    public function getAttributeList(&$input,$category_id)
    {
        //where条件拼接
        ...
        $builder= DB::table($this->table.' as ad')
            ->select('ad.id','ad.id as attribute_definition_id','ad.key','ad.name','ad.label','ad.is_visible_table','ad.is_man_haur_param','ad.range','ad.datatype_id',
                ...
                'uu.commercial as unit','ad.is_searchable')
...
//region  删
    ...
    public function destroy($id)
    {
        ...
        $has=$this->isExisted([[$this->apiPrimaryKey,'=',$id]],config('alias.ma'));
...
//endregion
    ...
    public function getAttributesByCategoryId($category_id)
    {
        ...
        $obj_list= DB::table($this->table)
            ->select('id','id as attribute_definition_id','key','name')
