...
function getPickingList(){
    ...
    AjaxClient.get({
        url: URLS['work'].getShopMergerPickingList+"?"+_token+urlLeft,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
                        ...
                        <div class="el-item-show">
                            ...
                            <ul class="el-item-hide">
                                ...
                                <li>
                                    ...
                                    <div class="el-form-item" id="send_depot">
                                        <div class="el-form-item-div">
        ...
        complete: function(){
            ...
            if(ajaxDataStr !== undefined && ajaxDataStr !== ''){
                ...
                $("#out_material_code").val(tansferNull(decodeURI(ajaxStr.out_material_code)))
                ...
                if(ajaxStr.send_depot_id>0){
                    $('#send_depot').val(ajaxStr.send_depot_name+"（"+ajaxStr.send_depot_code+"）").data('inputItem',{id:ajaxStr.send_depot_id,name:ajaxStr.send_depot_name}).blur();
                }
...
function createHtml(data){
    ...
    if(data&&data.results&&data.results.length){
        data.results.forEach(function(item,index){
            ...
            if(item.info.length>0){
                so_po_wo_html = creatSPWHtml(item.info);
            }
            trs+= `
			<tr data-id="${item.id}" class="material_item">
			<td>
			    <span class="el-checkbox_input el-checkbox_input_item" id="check_input_${item.id}" data-id="${item.id}" data-status="${item.status}">
			...
			<td>${tansferNull(item.get_depot_name)}</td>
    ...
    var thtml=`<div class="wrap_table_div">
            <table id="work_order_table" class="sticky uniquetable commontable" style="table-layout：fixed">
                    <thead>
                        <tr>
...
function createFineProducedHtml(data) {
    ...
    if (data && data.results && data.results.length) {
        data.results.forEach(function (item, index) {
            ...
            var checkedHtml = '',changeFactoryHtml='';
        ...
        trs = '<tr><td colspan="17" style="text-align:center">暂无数据</td></tr>';
    }
...
function creatSPWHtml(data){
    ...
    if(data&&data.length){
        data.forEach(function(item,index){
            trs+= `
                    ...
                    <td>${tansferNull(item.po_code)}</td>
                    <td>${tansferNull(item.wo_code)}</td>
    ...
    var thtml=`<div class="wrap_table_div">
            <table id="work_order_table_item" class="sticky uniquetable commontable" style="border: 1px solid #ccc;">
                <thead>
                    <tr>
                        <th class="left nowrap tight">销售订单号/行项号</th>
                        <th class="left nowrap tight">生产订单号</th>
...
function bindEvent() {
    ...
    $('body').on('click','#searchForm .submitPick',function(e){
        ...
        var $send_depot=$('#send_depot');
        var send_depot_id=$send_depot.data('inputItem')==undefined||$send_depot.data('inputItem')==''?'':
            $send_depot.data('inputItem').name==$send_depot.val().replace(/\（.*?）/g,"").trim()?$send_depot.data('inputItem').id:'';
        var send_depot_name=$send_depot.data('inputItem')==undefined||$send_depot.data('inputItem')==''?'':
            $send_depot.data('inputItem').name==$send_depot.val().replace(/\（.*?）/g,"").trim()?$send_depot.data('inputItem').name:'';
        var send_depot_code=$send_depot.data('inputItem')==undefined||$send_depot.data('inputItem')==''?'':
            $send_depot.data('inputItem').name==$send_depot.val().replace(/\（.*?）/g,"").trim()?$send_depot.data('inputItem').code:'';
        ...
        if(!$(this).hasClass('is-disabled')){
            ...
            pickAjaxData={
                ...
                send_depot_code: send_depot_code,
    ...
    $('body').on('click','#searchForm .resetPick',function(e){
        ...
        parentForm.find('#send_depot').val('').data('inputItem')=='';
    ...
    $('body').on('click','#combine',function (e) {
        ...
        if(choose_arr.length>0){
            AjaxClient.get({
                url: URLS['order'].checkShopPicking + _token + "&work_order_ids=" + choose_arr.join(','),
                dataType: 'json',
                beforeSend: function () {
                    layerLoading = LayerConfig('load');
                },
                success: function (rsp) {
                    ...
                    window.location.href = '/WorkOrder/combineShopItems?work_order_ids='+choose_arr+'&type='+type+'&workbench_no='+workbench_no;
                },
                fail: function (rsp) {
                    layer.close(layerLoading);
                    layer.close(layerModal);
    ...
    $('body').on('click','.el-checkbox_input_item',function(e){
        ...
        if($(this).hasClass('is-checked')){
            ...
            var s_index=status_arr.indexOf(status);
            mr_arr.splice(index,1);
            status_arr.splice(s_index,1);
        }
    });
    $('body').on('click','#check_input_item_all',function(e){
        ...
        if($(this).hasClass('is-checked')){
            ...
            $('#work_order_table .table_tbody .material_item').each(function (k,v) {
                ...
                var status=mr_arr.indexOf($(v).find('.el-checkbox_input_item').attr('data-status'));
                mr_arr.splice(index,1);
    ...
    $('body').on('click','.storage_all',function (e) {
      ...
      if(flag){
        AjaxClient.post({
            url: URLS['work'].shopMergerPickingInStorageBatch,
            data:{
              _token:"8b5491b17a70e24107c89f37b1036078",
              material_requisition_id_batch:mr_arr.join(',')
            },
            dataType: 'json',
            beforeSend: function () {
                layerLoading = LayerConfig('load');
