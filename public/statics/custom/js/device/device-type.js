...
function viewDeviceType(id,flag){
    AjaxClient.get({
        url: URLS['deviceType'].show+"?"+_token+"&id="+id,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
...
function deleteDeviceType(id){
    AjaxClient.get({
        url: URLS['deviceType'].destroy+"?"+_token+"&id="+id,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
...
function getUnique(flag,field,value,id,fn){
    ...
    var xhr=AjaxClient.get({
        url: URLS['deviceType'].unique+"?"+_token+urlLeft,
        dataType: 'json',
        beforeSend: function(){
            // layerLoading = LayerConfig('load');
        },
...
function bindEvent() {
    ...
    //输入框的相关事件
    $('body').on('focus', '.formDevice:not(".disabled") .el-input:not([readonly])', function () {
          ...
          <div class="el-form-item">
            ...
            success: function (layero, index) {
                ...
                layero.find('.submit').on('click', function () {
                    ...
                    originalBatchData = originalBatchData.replace(/\r/g, '').split("\n");
                    for (; i < originalBatchData.length; i++) {
                        var newText = originalBatchData[i];
                        newText = newText.split('\\');
                        if (newText.length === 2) {
                            newText[0] = pinyinUtil.getFirstLetter(newText[0], true) + '->' + newText[0];
                            newText[1] = pinyinUtil.getFirstLetter(newText[1], true) + '->' + newText[1];
                        }
                        else if (newText.length === 3) {
                            newText[0] = pinyinUtil.getFirstLetter(newText[0], true) + '->' + newText[0];
                            newText[1] = pinyinUtil.getFirstLetter(newText[1], true) + '->' + newText[1];
                            newText[2] = pinyinUtil.getFirstLetter(newText[2], true) + '->' + newText[2];
                        }
                        newText = newText.join('\\');
                        originalBatchData[i] = newText;
                    }
                    originalBatchData = originalBatchData.join("\n");
                    AjaxClient.post({
                        url: URLS['deviceType'].storeBatch,
                        dataType: 'json',
...
//查看和添加和编辑模态框
    function Modal(flag, deviceType, data) {
        ...
        flag === 'view' ? (btnShow = 'btnHide', readonly = 'readonly="readonly"') : (textareaplaceholder = '请输入描述，最多只能输入500字符', flag === 'add' ? title = '添加设备类型' : (title = '编辑设备类型', textareaplaceholder = '', noEdit = 'readonly="readonly"'));
        ...
        layerModal = layer.open({
          ...
          <div class="el-form-item">
            <div class="el-form-item-div">
                ...
                <input type="text" id="code" ${readonly} ${noEdit} data-name="编码" class="el-input" placeholder="编码由1-10个大写字母组成" value="${code}">
            </div>
            <p class="errorMessage" style="padding-left: ${labelWidth}px;display: block;"></p>
          </div>
          <div class="el-form-item">
