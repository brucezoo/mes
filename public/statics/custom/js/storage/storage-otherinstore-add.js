...
validatorToolBox = {
  ...
  checkIndentCode: function (name) {
    var value = $('.basic_info').find('#' + name).val().trim();
    return  (indentCodeCorrect = 1, !0);
  },
  checkWorkorderCode: function (name) {
    var value = $('.basic_info').find('#' + name).val().trim();
    return (workorderCodeCorrect = 1, !0);
  }
},
...
function getUnique(field, value, fn) {
  ...
  var xhr = AjaxClient.get({
    url: URLS['otherInstore'].unique + _token + urlLeft,
    dataType: 'json',
    beforeSend: function () {
      // layerLoading = LayerConfig('load');
    },
...
function addinformation() {
  ...
  $('.tr-item').each(function (item) {
	...
	if (data.depot_id == undefined){
		layer.alert('请选择其他入库仓储地！');
		_flag = 0;

	} else if ($(this).find('.usage_number').val().trim() == ''){
		layer.alert('请填写入库数量！');
		_flag = 0;
	} else{
		var obj = {
...
function createHtml(ele, data) {
  ...
  data.forEach(function (item, index) {
    ...
    <td><input type="text" class="bom-ladder-input batch" style="width: 100px;" value=""></td>
...
function bindEvent() {
  ...
  $('body').on('click', '.submit.save', function () {
    if (!$(this).hasClass('is-disabled')) {
      ...
      if (codeCorrect && indentCodeCorrect && workorderCodeCorrect) {

        if(remark == '') {
            layer.alert('请输入入库原因！');
        } else {
			let add = addinformation();
			console.log(111);
			if(add != 0 ) {
				addOtherInstore({
					code: code,
                        ...
                        <table class="bom_table">
                          <thead>
                            <tr>
                              ...
                              <th class="thead" style="text-align: center;">创建人</th>
                              <th class="thead" style="text-align: center;">材料类别</th>
                            </tr>
                          </thead>
