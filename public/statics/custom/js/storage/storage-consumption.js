...
function getOtherOutstoreList(){
	...
	AjaxClient.get({
		url: URLS['otherOutstore'].getConsumeList+_token+urlLeft,
		dataType:'json',
		beforeSend: function(){
...
function deleteOtherOutstore(id){
    AjaxClient.get({
        url: URLS['storageConsumption'].delStorageConsume+"id="+id+"&"+_token,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
...
//推送
function auditConsumption(id){
    var data={
        id: id,
        _token: TOKEN
    };
    AjaxClient.post({
        url: URLS['storageConsumption'].pushToSAP,
        data: data,
        dataType: 'json',
        beforeSend: function(){
            layer.close(layerLoading);
...
function createHtml(ele,data){
	var viewurl=$('#storageConsumption_view').val();
    ele.html('');
	editurl=$('#storageConsumption_edit').val();
		...
		var tr=`
		    <tr>
                ...
                <td>${tansferNull(item.code)}</td>
		            <td>${tansferNull(item.costcenter_name)}</td>
                <td>${tansferNull(item.factory_name)}</td>
                <td>${tansferNull(item.shop_name)}</td>
                <td>${tansferNull(item.employee_name)}</td>
                <td>${tansferNull(item.type==1?'共耗':'共耗冲销')}</td>
                <td>${tansferNull(item.status==1?'未推送':'已推送')}</td>
		        <td class="right nowrap">
                 ...
                 ${tansferNull(item.status==1?`<button data-id="${item.id}" class="button pop-button auditing">推送</button>`:'')}
	        </tr>
		`;
		ele.append(tr);
