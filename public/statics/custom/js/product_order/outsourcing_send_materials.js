...
function setAjaxOutData() {  
  ...
  if (ajaxDataStr !== undefined && ajaxDataStr !== '') {
      try{
          ajaxOutData=JSON.parse(decodeURIComponent(ajaxDataStr).substring(1));
...
function getOutPickingList(){
    ...
    AjaxClient.get({
        ...
        success: function(rsp){
            ...
            //     layerLoading = LayerConfig('load');
            // }
            ajaxOutData.itemPageNo=itemPageNo;
            ajaxOutData.status_type=status_type;
            window.location.href = '#' + encodeURIComponent(JSON.stringify(ajaxOutData));
            var totalData=rsp.paging.total_records;
...
function createItemHtml(data){
    ...
    if(data&&data.results&&data.results.length){
        data.results.forEach(function(item,index){
            ...
            trs+= `
				<tr>
				  <td><input type="checkbox" lay-skin="primary " data-id="${item.id}" class="check"></td>
                ...
                <!--  <td>${halfFinishedProductCode}</td>
                  ...
                  <td class="right">         
                    ${item.status == 0 ?`<a class="button pop-button view" href="/Outsource/sendOutsourceOrder?id=${item.id}">${item.type==3?'实退':'实发'}</a>`:`<a class="button pop-button view" href="/Outsource/sendOutsourceOrder?id=${item.id}">查看</a>`}
                  </td>
                </tr>
                `;
...
function getTab(str) {
	AjaxClient.get({
		url: '/OutWorkShop/batchOutdeliverylist' + "?" + _token + '&ids=' + str,
		dataType: 'json',
		beforeSend: function () {
			layerLoading = LayerConfig('load');
		},
...
$('body').on('click', '#ok', function () {
	...
	for(let i=0; i<table.length; i++) {
		...
		iAjax.push({
			...
			items: {
				...
				qty: item.qty,
				lot: item.lot,
				actual_send_qty: $(table[i]).find('.fl_qty').val(),
	...
	AjaxClient.post({
        ...
        url: '/OutWorkShop/batchOutdelivery',
        dataType: 'json',
        data: {
...
$('body').on('change', '.fl_qty', function() {
	if(Number($(this).val()) > Number($(this).parent().parent().find('#kc').attr('data-id'))) {
		layer.alert('发料数量不能大于库存数量！');
	}
})