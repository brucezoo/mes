  ...
  data.forEach(function (item) {
    ...
    var prohtml = `<tr>
            ...
            <td class="nowrap">
                ...
                ${item.status !== 0 ? `<button class="button pop-button notpublish" style="margin-bottom:5px;" data-id="${item.product_order_id}">撤回发布</button></br>` : ''}
                ${editBtn}
                ${flag == true ? `<button class="button pop-button republish" data-id="${item.product_order_id}">重新发布</button>` : ''}
...
function bindEvent() {
  ...
  $('body').on('click', '.item_close', function (e) {
    ...
    layer.confirm('将执行开启操作', {
      icon: 3, title: '开启生产订单', offset: '250px', end: function () {
  ...
  $('body').on('click', '.item_open', function (e) {
    ...
    var ele = $(this);
    AjaxClient.get({
      url: URLS['pro'].checkCanDelete + "?" + _token + "&product_order_id=" + poID,
      dataType: 'json',
      beforeSend: function () {
        layerLoading = LayerConfig('load');
      },
      success: function (rsp) {
        layer.close(layerLoading);
        if (rsp.results.status == 0) {
          var _html = '', warning = rsp.results.warning;
          if (warning && warning.length) {
            warning.forEach(function (item, index) {
              _html += `<p>${item}</p>`
            })
          }
          LayerConfig("fail", _html);
          // layer.confirm(_html, {
          //   icon: 3, title: '强制关闭生产订单',btn:['强制关闭','取消'], offset: '250px', end: function () {
          //     $('.uniquetable tr.active').removeClass('active');
          //   }
          // }, function (index) {
          //   layer.close(index);
