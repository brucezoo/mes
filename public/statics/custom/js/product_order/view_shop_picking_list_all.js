...
function getPickView() {
  ...
  AjaxClient.get({
    url: URLS['order'].getShopMergerPickingDetail + "?" + _token + "&material_requisition_id=" + id,
    dataType: 'json',
    beforeSend: function () {
...
function deleteItemSpeacil(id) {
  AjaxClient.post({
    url: URLS['work'].deleteShopMergerPickingLine,
    data: {
      item_id: id,
      _token: TOKEN
    },
...
//删除车间未发料行项
function deleteShopPickingNotSendLine() {
  AjaxClient.post({
    url: URLS['work'].deleteShopPickingNotSendLine,
    data: {
      picking_id: id,
      _token: TOKEN
    },
...
function showInItem(code, line, data, status, push_type) {
  ...
  data.forEach(function (item, index) {
    ...
    var tr = `
            ...
            <td>${((status == 2 || status == 3) && item.batches.length == 0) ? `<i class="fa fa-trash oper_icon delete_speacil" title="删除" data-id="${item.item_id
        }
