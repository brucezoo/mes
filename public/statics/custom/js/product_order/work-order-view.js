...
function getworkOrderView(id) {
	AjaxClient.get({
		...
		fail: function (rsp) {
			layer.close(layerLoading);
			layer.msg('获取工单详情失败，请刷新重试', {icon:5});
		}
	}, this)
}
...
function bindEvent() {
	...
	$('body').on('click', '#printWt', function (e) {
        ...
        $("#dowPrintWt").print({
            mediaPrint:false,
            stylesheet:'http://'+url+'/statics/custom/css/work_show_print.css'
        });
    });
}
...
//             material_in_html = cpreviewAttr(material_in, 'in');
//         } else {
//             material_in_html = `<span class="no_material">无</span>`;
//         }
//         if (material_out.length) {
//             material_out_html = cpreviewAttr(material_out, 'out');
//         } else {
//             material_out_html = `<span class="no_material">无</span>`;
//         }
//
//         // var material_in_html = '';
//         // material_in.forEach(function (initem) {
...
// 		}
// 		var _thml = `
// 					<div style="border-bottom: 1px solid #F0F1F1;" class="route_preview_container">
// 						<h4 style="color: #0510FB;">步骤${item.index}：${item.name}</h4>
// 						<div style="display: flex;flex-direction: row;flex-wrap: wrap; justify-content: flex-start;">
// 							<div style="flex: 1;color: #3DC08A;">消耗品</div>
// 							<div style="flex: 11;">
