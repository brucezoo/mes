...
function city() {
    AjaxClient.get({
        ...
        success: function (rsp) {
            ...
            data.forEach((item) => {
                var option = `<option value="${item.countroyId}">${item.countroyName}</option>`;
                $('#city').append(option);
            });
        },
...
function createHtml(ele, data) {
    ...
    data.forEach(function (item, index) {
        ...
        var tr = `
            <tr class="tritem" data-id="${item.drawing_id}">
                <td><input type="checkbox" name="like[read]"></td>
                ...
                <td>${item.ctime}</td> 
                <td class="right">
                <button data-id="${item.drawing_id}" class="button pop-button push" style="display: ${item.is_pushed==0?'inline-block':'none'}">推送</button>
                <button data-id="${item.drawing_id}" data-src="${imgSrc}" data-attribute="${item.has_attribute}"  class="button pop-button view">查看</button>
                <a class="link_button" style="border: none;padding: 0;" href="${editurl}?id=${item.drawing_id}"><button data-id="${item.drawing_id}" class="button pop-button edit">编辑</button></a>
    ...
    window.localStorage.setItem('arrData', JSON.stringify(data));
    allChoice();
}
...
function allChoice() {  
    // console.log(data);
    var datas = JSON.parse(window.localStorage.getItem('arrData'));
        ...
        new Promise(function (resolve, reject) {
            ...
            datas.forEach((item, index) => {
                if(checks[index].checked == true) {
                    arr.push(item.drawing_id);
                }
            })

            window.localStorage.setItem('arrAdd',JSON.stringify(arr) );
           return resolve();
        }).then(function() {
            arr  = JSON.parse( window.localStorage.getItem('arrAdd'));
            console.log(arr);
            // console.log(arr,11);
            if(arr.length == 0) {
                layer.msg('请进行勾选！', { time: 3000, icon: 5 }); 
            } else {
                ...
                $('#loada').attr('href', '/CareLabel/getImagesUrl' +
                    '? _token = 8b5491b17a70e24107c89f37b1036078' + '&data=' + string + '& sell_code=' + $('#sale_order_code').val());
                // AjaxClient.get({
                //     url: URLS['city'].img,
                //     data: {
                //         data:string,
                //         sell_code: $('#sale_order_code').val(),
                //         _token: TOKEN
                //     },
                //     dataType: 'json',
                //     fail: function (rsp) {
...
function pushCareLable(id) {
    AjaxClient.post({
        ...
        fail:function (rsp) {
            layer.close(layerLoading);
            LayerConfig('fail', '失败！' + rsp.message);
        }
    });
}

$('body').on('click','.push',function(){
    var drawing_id = $(this).attr('data-id');
    pushCareLable(drawing_id);
})
...
function bindEvent() {
    ...
    $('body').on('click', '#searchForm .submit:not(".is-disabled")', function (e) {
        ...
        if (!$(this).hasClass('is-disabled')) {
            ...
            ajaxData = {
                ...
                line_project_code: encodeURIComponent(parentForm.find('#line_project_code').val().trim()),
                material_code: encodeURIComponent(parentForm.find('#material_code').val().trim()),
                order: 'desc',
                sort: 'ctime',
                country_id: encodeURIComponent(parentForm.find('#city').val().trim())
            };
    ...
    $('body').on('click', '#searchForm .reset:not(.is-disabled)', function (e) {
        ...
        parentForm.find('#line_project_code').val('');
        parentForm.find('#material_code').val('');
        parentForm.find('#city').val('').siblings('.el-input').val('--请选择--');
