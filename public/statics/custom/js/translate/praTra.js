var translate = window.sessionStorage.getItem('pra');
...
function getList() {
	$('#tbody').html('');
	AjaxClient.get({
		url: '/Language/getPracticeList' + '?' + _token + '&page_no=' + str.page_no + '&page_size=' + str.page_size + '&languageCode=' + str.languageCode,
		dataType: 'json',
		beforeSend: function () {
			layerLoading = layer.load(0, { shade: [0.3, '#bbb'] });
		},
...
function getTr(data) {
	let tr = `
        <tr>
            <td><input type="checkbox" data-id='${data.rpfId}'  class = "input"></td>
            <td data-id="${data.rpfCode}" id="code">${data.rpfCode}</td>
            <td>${data.rpfName}</td>
			<td><input type="text" id="lName" value="${data.lanName == null ? '' : data.lanName} "></td>
			<td>${data.rpfDescription}</td>
			<td><input type="text" id="lDes" value="${data.lanDescription == null ? '' : data.lanDescription} "></td>
        </tr>
    `;
...
$('#save').on('click', function () {
	...
	for (let i = 0; i < check.length; i++) {
		if (check[i].checked == true) {
			st = {
				rpfId: $(check[i]).attr('data-id'),
				languageCode: $('#list').val(),
				name: $(check[i]).parent().parent().find('#lName').val(),
				rpfCode: $(check[i]).parent().parent().find('#code').attr('data-id'),
				description: $(check[i]).parent().parent().find('#lDes').val(),
			}
			arr.push(st);
		}
	}
	if (arr.length != 0) {
		...
		AjaxClient.post({
			url: '/Language/practiceLanguage' + '?' + _token,
			dataType: 'json',
			data: datas,
...
$('#printOut').on('click', function () {
	...
	res.then(function (a) {
		...
		$('#printOuta').attr('href', '/Language/exportPractice? _token = 8b5491b17a70e24107c89f37b1036078'  + '&languageCode=' + languageCode)
	})
})
