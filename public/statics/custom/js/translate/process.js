var translate = window.localStorage.getItem('translates');
...
layui.use('upload', function () {
    ...
    upload.render({
        elem: '#test8'
        , url: '/Language/importOperationExcel'
        , auto: false
...
function getCount() {
    AjaxClient.get({
        url: URLS['working'].get + '?' + _token + '&page_no=' + str.page_no + '&page_size=' + str.page_size + '&languageCode=' + str.languageCode,
        dataType: 'json',
        fail: function (rsp) {
...
function getList() {
    ...
    AjaxClient.get({
        ...
        fail: function (rsp) {
            ...
            window.localStorage.setItem('choices', JSON.stringify(data));
            choice();
        },
    }, this);
}
...
function getTr(data) {
    ...
    let tr = `
        <tr>
            ...
            <td><input type="text" id="lName" value="${data.operationLanName == null ? '' : data.operationLanName} "></td>
            <td>${data.abilityName}</td>
            <td><input type="text" id="lDes" readonly="readonly"  value="${data.abilityLanName == null ? '' : data.abilityLanName}"></td>
        </tr>
    `;
...
$('#save').on('click', function () {
    ...
    let date = JSON.parse(window.localStorage.getItem('choices'));

    for (let i = 0; i < check.length; i++) {
        if (check[i].checked == true) {
            st = {
                operationId: date[i].operationId,
                languageCode: $('#list').val(),
...
$('#printOut').on('click', function () {
    ...
    res.then(function (a) {
        ...
        $('#printOuta').attr('href', '/Language/exportOperationExcel? _token = 8b5491b17a70e24107c89f37b1036078' + '&data=' + data + '&languageCode=' + languageCode)
    })
})
