...
function bindEvent() {
    ...
    $('body').on('click', '#searchForm .reset', function (e) {
        e.stopPropagation();
        var parentForm = $('#searchForm');
        parentForm.find('#type_code').val('').siblings('.el-input').val('--请选择--');
    ...
    $('body').on('click', '.el-form-item-div .export', function (e) {
        ...
        let url = "/OutMachineZy/export?" + urlLeft;
...
function createHtml(ele, data) {
    ...
    data.forEach(function (item, index) {
        var tr = `
			...
			<td class="right">
			 ${item.editStatus == 1 ? `<a class="button pop-button view" href="/Outsource/editOutsource?id=${item.id}&type=${item.type_code}&flag=1">编辑</a>` : ''}
