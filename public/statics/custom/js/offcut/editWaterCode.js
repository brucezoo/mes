...
$(function () {
	...
	if (url.indexOf('editWaterCode') > -1) {
		mActionFlag = 'edit';
...
function getImage(id) {
	var xhr = AjaxClient.get({
		url: '/Image/waterCodeShow' + "?" + _token + "&drawing_id=" + id,
		dataType: 'json',
		beforeSend: function () {
			layerLoading = LayerConfig('load');
		},
...
function getCareLable(id) {
	AjaxClient.get({
		...
		success: function (rsp) {
			...
			console.log(rsp)
			if (rsp.results.rdcl) {
				crateCareLableHtml($('.careLable_table .table_tbody'), rsp.results.rdcl);
			}
			// } else {
			// 	var tr = `<tr >
...
function crateCareLableHtml(ele, data) {
	...
	data.forEach(function (item) {
		...
		var arr = item.page_division.split(",")
		...
		var tr = `<tr data-id="${item.care_label_id}">
						...
						<td><input type="text" id="_left" readonly="readonly" value="${left}" style="width:50px;" >-<input readonly="readonly" value = "${right}" id="_right" type="text" style="width:50px;"></td>
                        <td><input type="text" placeholder="请输入备注" class="remarkMsg" data-id="${item.id}" value="${item.remark}"></td>
                    </tr>`;
		ele.append(tr);
		...
		if (!$(this).hasClass('is-disabled')) {
			...
			if (correct) {
				...
				var data = {},
					...
					items = {
						"care_label_id": $('.remarkMsg').attr('data-id'),
						"sale_order_code": $('.sale_order_code').val(),
						"line_project_code": $('.line_project_code').val(),
