...
function bindEvent() {
  ...
  $('.uniquetable').on('click', '.edit', function () {
    ...
    getInvalidItemInfo($(this).attr("data-id"));
  });
  ...
  //添加和编辑的提交
  $('body').on('click', '.formInvalidItem:not(".disabled") .submit', function (e) {
    e.stopPropagation();
    if (!$(this).hasClass('is-disabled')) {
      var parentForm = $(this).parents('#addInvalidItem_form');
  ...
  //输入框的相关事件
  $('body').on('focus', '.formInvalidItem:not(".disabled") .el-input:not([readonly])', function () {
    $(this).parents('.el-form-item').find('.errorMessage').removeClass('active').html("");
  }).on('blur', '.formInvalidItem:not(".disabled") .el-input:not([readonly])', function () {
...
function Modal(data,flag) {
  ...
  layerModal = layer.open({
    ...
    resize: false,
    move: false,
    content: `<form class="formModal formInvalidItem" id="addInvalidItem_form">
                  ...
                  <div class="el-form-item">
                      <div class="el-form-item-div btn-group" style="margin-top: 5px;">
                          <button type="button" class="el-button el-button--primary submit ${flag!=1?'addInvalidItem':''}">确定</button>
                      </div>
                  </div>   
              </form>`,
    success: function () {
...
//查看不良项目
function getInvalidItemInfo(id) {
  AjaxClient.get({
