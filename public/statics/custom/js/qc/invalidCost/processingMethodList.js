...
//获取处理方式列表
function getProcessingMethodList() {
  $('.table_tbody').html('');
  var urlLeft = '';
...
function bindPagenationClick(total, size) {
  ...
  $('#pagenation').pagination({
    ...
    callback: function (api) {
      ...
      getProcessingMethodList(status);
    }
  });
}
...
function bindEvent() {
  ...
  $('.uniquetable').on('click', '.edit', function () {
    ...
    getProcessingMethodInfo($(this).attr("data-id"));
  });
  ...
  $('body').on('click', '#typeList .el-select-dropdown-item', function (e) {
    ...
    var flag = $("#flag").val();
    getProcessingMethodCode(code_id, flag);
  })
  ...
  $('body').on('click', '#codeListItem .el-select-dropdown-item', function (e) {
    ...
    //获取编码select列表
    var code_id = $(this).attr('data-id');
    getProcessingMethodCodeNumber(code_id);
  })
  ...
  //添加和编辑的提交
  $('body').on('click', '.formProcessingMethod:not(".disabled") .submit', function (e) {
    e.stopPropagation();
    if (!$(this).hasClass('is-disabled')) {
      var parentForm = $(this).parents('#addProcessingMethod_form');
      ...
      if (nameCorrect) {
        ...
        $(this).hasClass('addProcessingMethod') ? (
          addProcessingMethod({
              name: name,
              type: type,
              _token: TOKEN
  ...
  //输入框的相关事件
  $('body').on('focus', '.formProcessingMethod:not(".disabled") .el-input:not([readonly])', function () {
    $(this).parents('.el-form-item').find('.errorMessage').removeClass('active').html("");
  }).on('blur', '.formProcessingMethod:not(".disabled") .el-input:not([readonly])', function () {
...
function Modal(data,flag) {
  ...
  layerModal = layer.open({
    ...
    resize: false,
    move: false,
    content: `<form class="formModal formProcessingMethod" id="addProcessingMethod_form">
                  ...
                  <div class="el-form-item">
                      <div class="el-form-item-div btn-group" style="margin-top: 5px;">
                          <button type="button" class="el-button el-button--primary submit ${flag!=1?'addProcessingMethod':''}">确定</button>
                      </div>
                  </div>   
              </form>`,
    success: function () {
...
//添加处理方式
function addProcessingMethod(data) {
  AjaxClient.post({
...
//编辑处理方式
function editProcessingMethod(data){
  AjaxClient.post({
...
//查看处理方式
function getProcessingMethodInfo(id) {
  AjaxClient.get({
...
//删除处理方式
function deleteProcessingMethod(id) {
  AjaxClient.post({
