...
function getInvalidCostList() {
  ...
  AjaxClient.get({
    url: URLS['invalidCost'].getHandleInvalidOfferList + _token + urlLeft,
    dataType: 'json',
    beforeSend: function () {
  ...
  data.forEach(function (item, index) {
    ...
    var tr = `
		    <tr>
                ...
                <td style="width:250px;word-wrap:break-word;word-break:break-all;">${materials}</td>
                ...
                <td>${tansferNull(item.is_third_handle == 0 ? '待核算' : item.is_third_handle == 1 ? '核算中' : '已核算')}</td>
                <td>${tansferNull(item.type==0?'失效提报':'索赔单')}</td>
                <td>${tansferNull(dateFormat(item.ctime,'Y-m-d H:i:s'))}</td>
                <td class="right nowrap">
                  <a href="/QC/invalidCostIndex?id=${item.id}"><button data-id="${item.id}" class="button pop-button edit">编辑</button></a>
                  ${item.type==0 ? `<button data-id="${item.id}" class="button pop-button push">推送</button>`: ''}
                </td>
	        </tr>
		`;
    ele.append(tr);
...
function handleMethod(ids) {
  var data = {
    id_batch: ids.join(),
    _token: TOKEN
  };
  AjaxClient.post({
    url: URLS['invalidCost'].dealInvalidOfferById,
    data: data,
    dataType: 'json',
    beforeSend: function () {
      layerLoading = LayerConfig('load');
...
function bindEvent() {
  ...
  $('body').on('click', '#searchForm .handle', function (e) {
    var ids = [];
    ...
    if (ids && ids.length) {
      ...
      layer.msg('请先选择已处理的提报单！', { icon: 5, offset: '250px', time: 1500 });
    }
  })
  ...
  $('body').on('click', '#searchForm .submit', function (e) {
    ...
    if (!$(this).hasClass('is-disabled')) {
      ...
      ajaxData = {
        ...
        is_third_handle: encodeURIComponent(parentForm.find('#is_third_handle').val().trim()),
  ...
  $('body').on('click', '#searchForm .reset', function (e) {
    ...
    parentForm.find('#is_third_handle').val('').siblings('.el-input').val('--请选择--');
