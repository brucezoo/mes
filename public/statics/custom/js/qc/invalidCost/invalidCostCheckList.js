  ...
  data.forEach(function (item, index) {
    ...
    var tr = `
		    <tr>
                ...
                <td>${tansferNull(item.is_handle == 0 ? '其他' : item.is_handle == 1 ? '待处理' : '已处理')}</td>
                <td>${tansferNull(dateFormat(item.ctime,'Y-m-d H:i:s'))}</td>
                <td class="right nowrap">
                  <a href="/QC/invalidCostIndex?id=${item.id}&menu=2"><button data-id="${item.id}" class="button pop-button edit">编辑</button></a>
                </td>
	        </tr>
		`;
    ele.append(tr);
