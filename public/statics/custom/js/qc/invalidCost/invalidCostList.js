...
function getInvalidCostList() {
  ...
  AjaxClient.get({
    url: URLS['invalidCost'].getInvalidOfferList + _token + urlLeft,
    dataType: 'json',
    beforeSend: function () {
  ...
  layerModal = layer.open({
    ...
    <input type="hidden" id="pushIds" value="${ids}">
            ...
            <div class="el-form-item-div btn-group">
                <button type="button" class="el-button cancle" style="padding:10px 20px;">取消</button>
                <button type="button" class="el-button el-button--primary submit" style="padding:10px 20px;">确定</button>
            </div>
          </div>
        </form>` ,
    success: function (layero, index) {
      getLayerSelectPosition($(layero));
      $('#judge_person_id').autocomplete({
        url: URLS['complaint'].judge_person + "?" + _token + "&page_no=1&page_size=50"
      });
      $('#judge_person_id').each(function (item) {
        var width = $(this).parent().width();
        $(this).siblings('.el-select-dropdown').width(width);
...
layui.use('upload', function () {
	...
	upload.render({
		elem: '#test8'
		, url: '/InvalidCost/importInvalidOffer'
		, auto: false
