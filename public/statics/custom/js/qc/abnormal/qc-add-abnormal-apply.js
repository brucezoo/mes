...
$('body').on('click','.saveMsg',function(){
    ...
    if(abnormalApplyMsg.length == 0){
        ...
        AjaxClient.post({
            url: URLS['check'].storeApply+'?'+_token+'&key_persons='+key_persons+'&question_description='+question_description+
            '&temp_way='+temp_way+'&cause='+cause+'&final_method='+final_method+'&result_final_method='+result_final_method+'&end_remark='+end_remark+'&items='+items+'&drawings='+JSON.stringify(drawings),
            dataType: 'json',
            beforeSend: function(){
                layerLoading = LayerConfig('load');
            },
