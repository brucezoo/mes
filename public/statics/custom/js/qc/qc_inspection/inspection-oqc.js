  ...
  data.forEach(function (item, index) {
    ...
    var tr = `
            <tr class="tritem" data-id="${item.id}" style="background-color: ${copyFlag ? '#d0d000' : ''}">
                ...
                <td><input onkeyup="value=value.replace(/\\-/g,'')" data-id="${item.id}" class="mc-change" value="${tansferNull(item.LIFNR)}" style="border: none;color: #393939;font-size: 12px;"></input></td>
                <td><input type="number" onkeyup="value=value.replace(/\\-/g,'')" data-id="${item.id}" class="order_number_val deal" value="${tansferNull(item.order_number)}" style="border: none;color: #393939;font-size: 12px;"></td>
                ...
                <td class="right">
                    ...
                    <button class="button pop-button view viewProcess" data-operation-id="${item.operation_id}" data-sales-order-code="${item.sales_order_code}" data-sales-order-project-code="${item.sales_order_project_code}" data-id="${item.work_order_id ? item.work_order_id : item.sub_order_id}">工艺文件</button>
    ...
    content: `<form class="attachmentForm formModal formAttachment" autocomplete="off" id="claim_form"> 
            ...
            <div class="el-form-item">
                <div class="el-form-item-div">
                    ...
                    <textarea type="textarea" maxlength="500" id="DEFECT_DESC" rows="5" class="el-textarea" placeholder="请输入描述，最多只能输入500字符"></textarea>
                </div>
                <p class="errorMessage" style="padding-left: 100px;display: block;"></p>
              </div>
            <div class="el-form-item">
...
function copyCheck(id) {
  AjaxClient.get({
    ...
    success: function (rsp) {
      layer.close(layerLoading);
      layer.msg('复制成功', { icon: 1, offset: '250px', time: 1500 }, function () {
        getChecks();
      });
    },
    fail: function (rsp) {
      layer.close(layerLoading);
      layer.msg(rsp.message, { icon: 2, offset: '250px', time: 1500 }, function () {
        getChecks();
      });
    }
  }, this);
...
function updateLIFNR(id,LIFNR){
  AjaxClient.post({
    ...
    dataType: 'json',
    data: { 'id': id, 'sub_number': LIFNR, _token: TOKEN },
    beforeSend: function () {
      layerLoading = LayerConfig('load');
    },
    success: function (rsp) {
  ...
  $('body').on('click', '#searchForm .submit:not(".is-disabled")', function (e) {
    ...
    if (!$(this).hasClass('is-disabled')) {
      ...
      ajaxData = {
        ...
        status: parentForm.find('#check_status').val().trim(),
...
function updateOrderAmount(id, value) {
  AjaxClient.post({
    ...
    dataType: 'json',
    data: { 'id': id, 'order_number': value, _token: TOKEN },
    beforeSend: function () {
      layerLoading = LayerConfig('load');
    },
    success: function (rsp) {
...
function sumbitCheck(id) {
  ...
  AjaxClient.get({
    url: URLS['check'].pushFinishedproduct + "?" + _token + "&check_id=" + id,
    dataType: 'json',
    beforeSend: function () {
      layerLoading = LayerConfig('load');
    },
...
function Modal(flag,checkItems,id){
  ...
  if(flag=='single'){
      if(checkItems.check_res){
          ({result='',anomalies='',workshop_id='',dispose='',department_id='',missing_items='',missing_items_id='',dispose_ideas='',unit='',unit_text='',commercial='',question_description='',scene='',missing_item_relational_id='',deadly='',seriousness='',slight='',grammage1='',grammage2='',grammage3='',gatewidth1='',gatewidth2='',gatewidth3='',dimensions1='',dimensions2='',dimensions3='',is_suremodify='',is_sureabnormal=''}=checkItems.check_res);
      }
  }else{
    if(checkItems.check_res){
      ({result='',anomalies='',workshop_id='',dispose='',department_id='',missing_items='',missing_items_id='',dispose_ideas='',unit='',unit_text='',commercial='',question_description='',scene='',missing_item_relational_id='',deadly='',seriousness='',slight='',grammage1='',grammage2='',grammage3='',gatewidth1='',gatewidth2='',gatewidth3='',dimensions1='',dimensions2='',dimensions3='',is_suremodify='',is_sureabnormal=''}=checkItems.check_res);
  ...
  if (department_id) {
    var departmentArr = department_id.split(','), department_val = '';
                ...
                <table class="checkbox el-item-show">
                    <tr>
                        ...
                        <td style="width: 400px;">
                            ...
                            <div class="el-form-item" style="display:inline-block;width:120px;min-width:120px;margin-left: 20px;">
                                    <div class="el-form-item-div" style="display:inline-block;width:120px;min-width:120px;" id="unitDiv">
                                        <div class="el-select-dropdown-wrap">
                                            <div class="el-select">
                                                <i class="el-input-icon el-icon el-icon-caret-top"></i>
                    ...
                    <tr> 
                        ...
                        <table class="sticky uniquetable  quality_table">
                            <thead>
                            <tr>
                                <th class="center nowrap tight" colspan="2">品质履历</th>                       
                            </tr>
                            </thead>
                            <tbody class="table_tbody">
                            </tbody>
