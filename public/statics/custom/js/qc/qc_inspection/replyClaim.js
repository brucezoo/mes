...
function getClaims() {
  ...
  $('.table_tbody').html('');
  AjaxClient.get({
    url: URLS['check'].replyPageindex + "?" + _token + urlLeft,
    dataType: 'json',
    beforeSend: function () {
  ...
  data.forEach(function (item, index) {
    var tr = `
            <tr class="tritem" data-id="${item.id}">            
                ...
                <td>${tansferNull(item.persons_arr)}</td>
                <td>${tansferNull(item.replyperson)}</td>
                <td class="right">
                    ${item.status == 1 ? `<button data-id="${item.id}" class="button pop-button send">推送</button>` : ''}
                    <a href="/Claim/reviewClaim?id=${item.id}" ><button data-id="${item.id}" class="button pop-button">查看</button></a>
                </td>
            </tr>
        `;
    ele.append(tr);
...
function bindEvent() {
  ...
  $('body').on('click', '#claim_form .review:not(.is-disabled)', function (e) {
    e.stopPropagation();
...
function review(){
  ...
  var data={
    'id':id,
    'status':status,
    'contentofreply':review_desc,
    _token:TOKEN
  }
  AjaxClient.get({
    url: URLS['check'].review,
    data:data,
    beforeSend: function () {
      layerLoading = LayerConfig('load');
    },
...
function getClaim(id) {
  ...
  AjaxClient.get({
    ...
    success: function (rsp) {
      ...
      if (rsp && rsp.results) {
        console.log(rsp)
        getClaimModal(rsp.results[0], rsp.results[0].groups,id);
      } else {
...
function getClaimModal(Info, data,id) {
  ...
  layerModal = layer.open({
    ...
    content: `<form class="attachmentForm formModal formAttachment" id="claim_form"> 
              <input type="hidden" id="claimId" value="${id}">
              ...
              <div class="el-form-item">
                <div class="el-form-item-div" id="specialPurchase">
                  <label class="el-form-item-label" style="width: 110px;">是否同意索赔<span class="mustItem">*</span></label>
                  <div class="el-radio-group">
                    <label class="el-radio">
                        ...
                        <span class="el-radio-label">同意索赔</span>
                    </label>
                    <label class="el-radio">
                        ...
                        <span class="el-radio-label">拒绝索赔</span>
                    </label>
                  </div>
                </div>
              </div>
              <div class="el-form-item">
                <div class="el-form-item-div">
                   <label class="el-form-item-label" style="width: 150px;">回复内容<span class="mustItem">*</span></label>
                    <textarea type="textarea" maxlength="500" id="review_desc" rows="5" class="el-textarea" placeholder="请输入回复内容，最多只能输入500字符"></textarea>                 
                </div>
              </div>
              <div class="el-form-item">
                  <div class="el-form-item-div btn-group" style="margin-top: 10px;">
                      <button type="button" class="el-button el-button--primary review">审核</button>
                  </div>
              </div>            
        </form>`,
