                    ...
                    <div class="apply">
                        ...
                        <div class="el-form-item">
                            <span class="el-form-item-div abnormal-item">
                                ...
                                <input type="text" class="el-input abnormalApply-input reject_ratio" placeholder="请输入不合格率" value="${val.reject_ratio}">
                            </span>
                            <span class="el-form-item-div abnormal-item">
                                <label class="el-form-item-label" style="margin-bottom: 0px;width: 120px;margin-left: 65px;">
...
$('body').on('click', '.saveMsg', function () {
  ...
  var items = JSON.stringify(abnormalApplyMsg),
    ...
    id = $('#special_purchase_id').val();

  if (abnormalApplyMsg.length == 0) {
    LayerConfig('fail', '至少要有一条检验单');
  } else {
    AjaxClient.get({
      url: URLS['deviation'].update + '?' + _token + '&key_persons=' + key_persons + '&question_description=' + question_description +
        '&cause=' + cause + '&items=' + items + '&id=' + id + '&drawings=' + JSON.stringify(drawings),
      dataType: 'json',
      beforeSend: function () {
        layerLoading = LayerConfig('load');
      },
