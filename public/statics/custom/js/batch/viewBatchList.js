...
function getWorkCenter() {
  ...
  AjaxClient.get({
    url: URLS['option'].getWorkCenter + _token,
    dataType: 'json',
    beforeSend: function () {
    ...
    data.results.list.forEach(function (item) {
      ...
      trs += `
            <tr>
                ...
                <td>${tansferNull(item.width)}</td>
                <td>${tansferNull(item.length)}</td>
                <td>${tansferNull(item.high)}</td>
                <td>${tansferNull(item.weight)}</td>
                <td class="right">
                  <button class="button pop-button view update-mile" data-item='${JSON.stringify(item)}'>填写米数</button>
                  <button class="button pop-button view get-weight" data-item='${JSON.stringify(item)}'>获取重量</button>
                  <button class="button pop-button view update-weight" data-item='${JSON.stringify(item)}'>填写重量</button>
                  <button class="button pop-button view view-qrcode" data-item='${JSON.stringify(item)}' data-id="${item.work_order_id}">生成二维码</button>
                </td>
			      </tr>
			`;
    })
        ...
        success: function (rsp) {
          ...
          layerModal = layer.open({
            ...
            content: `<form class="viewAttr formModal" id="viewattr">
                      ...
                      <div id="dowPrintWt" style="width: 10cm;height: 8cm;border: 1px;font-size:18px;color:#000;">
                        <div style="display: flex;">
                          <div style="flex: 2;height:">
                            ...
                            <div class="el-form-item-div" style='width:240px;'>
                              ...
                              <div style="font-size:16px;color:#000;">${data.material_name}</div>
                            </div>
                            <div class="el-form-item-div" style='width:240px;'>
