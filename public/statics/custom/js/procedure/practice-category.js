...
validatorToolBox={
    checkCode: function(name){
        var value=$('#addPracticeCategory_form').find('#'+name).val().trim();
...
function getUnique(flag,field,value,id,fn){
    ...
    var xhr=AjaxClient.get({
        url: URLS['practiceCategory'].unique+"?"+_token+urlLeft,
        dataType: 'json',
...
function showInvalidMessage(name,val) {
    $('.addPracticeCategory').find('#'+name).parents('.el-form-item').find('.errorMessage').html(val).addClass('active');
    $('.addPracticeCategory').find('.submit').removeClass('is-disabled');
}
...
function getCategoryData(){
    $('.table_tbody').html(' ');
    AjaxClient.get({
        url: URLS['practiceCategory'].list+'?'+_token,
        dataType: 'json',
        beforeSend: function(){
...
function addCategory(data) {
    AjaxClient.post({
        url: URLS['practiceCategory'].store,
        data: data,
        dataType: 'json',
        beforeSend: function(){
...
function categoryDelete(id) {
    AjaxClient.get({
        url: URLS['practiceCategory'].delete+'?'+_token+"&"+"practiceCategory_id="+id,
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
        success: function(rsp){
...
function categoryView(id,flag) {
    console.log(id);
    AjaxClient.get({
        url: URLS['practiceCategory'].show+'?'+_token+"&"+"practiceCategory_id="+id,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
        success: function(rsp){
            layer.close(layerLoading);
            getCategoryItemData(id,flag,rsp.results);
...
function editCategory(data) {
    AjaxClient.post({
        url: URLS['practiceCategory'].update,
        data: data,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = layer.load(2, {shade: false,offset: '300px'});
        ...
        fail: function(rsp){
            ...
            $('body').find('#addPracticeCategory_form').find('.submit').removeClass('is-disabled');
...
//获取select列表
function getCategoryItemData(id,flag,data){
...
//输入框的相关事件
    $('body').on('focus','.formMateriel:not(".disabled") .el-input:not([readonly])',function(){
        ...
        var flag=$('#addPracticeCategory_form').attr("data-flag"),
            name=$(this).attr("id"),
            id=$('#itemId').val();
...
//添加页面
    $('body').on('click','#practiceCategory_add.button',function(e){
        e.stopPropagation();
        getCategoryItemData(0,'add');
    });
...
//关闭弹窗
    $('body').on('click','.addPracticeCategory .cancle',function(e){
        e.stopPropagation();
        layer.close(layerModal);
    });
//树形表格展开收缩
...
//点击编辑
    $('.uniquetable').on('click','.button.pop-button.edit',function(){
        ...
        codeCorrect=!1;
        $(this).parents('tr').addClass('active');
        categoryView($(this).attr("data-id"),'edit');
    });
//查看
    $('.uniquetable').on('click','.button.pop-button.view',function(){
        $(this).parents('tr').addClass('active');
        categoryView($(this).attr("data-id"),'view');
    });
...
function showTypeModal(flag,Category,data) {
    var {id='',code='',name='',parent_id='',comment=''}={};
    if(data){
        ({id='',code='',name='',parent_id='',comment=''}=data)
    }
    ...
    var labelWidth=100,
        ...
        btnShow='btnShow',
        readonly='',
        shtml=selectHtml(Category,flag,parent_id),
        noEdit='';

    flag==='view'?(btnShow='btnHide',readonly='readonly="readonly"'):(flag==='edit'?(title='编辑做法分类',noEdit='readonly="readonly"'):title='添加做法分类');
    layerModal = layer.open({
        ...
        resize: false,
        move: false,
        content:`<form class="addPracticeCategory formModal formMateriel" id="addPracticeCategory_form" data-flag="${flag}">
                ...
                <div class="el-form-item">
                    <div class="el-form-item-div">
                        ...
                        <textarea type="textarea" maxlength="500" ${readonly} id="description" rows="5" class="el-textarea" placeholder="${flag=='view'?'':'请输入注释，最多只能输入500字'}">${tansferNull(comment)}</textarea>
                    </div>
                    <p class="errorMessage" style="padding-left: ${labelWidth}px; display:block;"></p>
                </div>
    ...
    children.forEach(function (item, index) {
        ...
        if(flag==='table'){
	        ...
	        <tr data-id="${item.id}" data-pid="${parent_id}" class="${className}">
	          <td>${item.code}</td>
	          <td>${span}</td>
	          <td><div>${item.comment.length>30?item.comment.substring(0,30)+'...':item.comment}</div></td>
	          <td class="right">
