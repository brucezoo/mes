...
function getUnique(flag,field,value,id,fn){
    var urlLeft='';
    if(flag==='edit'){
        urlLeft=`&field=${field}&value=${value}&lining_type_id=${id}`;
    }else{
        urlLeft=`&field=${field}&value=${value}`;
    }
    var xhr=AjaxClient.get({
        url: URLS['lining'].unique+"?"+_token+urlLeft,
        dataType: 'json',
...
function typeView(id,flag) {
    AjaxClient.get({
        url: URLS['lining'].show+'?'+_token+"&"+"lining_type_id="+id,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
...
function showTypeModal(flag,Type,data) {
    var {lining_type_id='',code='',name='',parent_id='',description=''}={};
    if(data){
        ({lining_type_id='',code='',name='',parent_id='',description=''}=data)
    }
    ...
    flag==='view'?(btnShow='btnHide',readonly='readonly="readonly"'):(flag==='edit'?(title='编辑面料分类',noEdit='readonly="readonly"'):title='添加面料分类');
    layerModal = layer.open({
        ...
        content:`<form class="addType formModal formMateriel" id="addType_form" data-flag="${flag}">
                <input type="hidden" id="itemId" value="${lining_type_id}">
...
function selectHtml(fileData,flag,value){
    ...
    itemSelect.length?(selectVal=itemSelect[0].name,parent_id=itemSelect[0].lining_type_id):
...
function treeHtml(fileData, parent_id, flag,value) {
    ...
    children.forEach(function (item, index) {
        ...
        var hasChild = hasChilds(fileData, item.lining_type_id);
        ...
        if(flag==='table'){
            _html += `
	        <tr data-id="${item.lining_type_id}" data-pid="${parent_id}" class="${className}">
	          <td>${item.code}</td>
	          <td>${span}</td>
	          <td><div>${item.description.length>30?item.description.substring(0,30)+'...':item.description}</div></td>
	          <td class="right">
                <button data-id="${item.lining_type_id}" data-pid="${parent_id}" class="button pop-button view">查看</button>
                <button data-id="${item.lining_type_id}" data-pid="${parent_id}" class="button pop-button edit">编辑</button>
                <button data-id="${item.lining_type_id}" data-pid="${parent_id}" class="button pop-button delete">删除</button>
              </td>
            </tr>
	        ${treeHtml(fileData, item.lining_type_id, flag)}
	        `;
        }else{

            item.lining_type_id==value?(itemSelect.push(item),selectedClass='selected'):null;


            _html += `
    		<li data-id="${item.lining_type_id}" data-pid="${parent_id}" data-code="${item.code}" data-name="${item.name}" class="${className} el-select-dropdown-item ${selectedClass}">${span}</li>
	        ${treeHtml(fileData, item.lining_type_id, flag,value)}
	        `;
        }
    });
    return _html;
