...
function emi2cRoutingGraph(parentElementName, routingGraph, width, height) {
    ...
    if (this.parent.length) {
        ...
        var download=$('<button class="btn" style="position: absolute;right: 10px;top: 10px; padding: 0 8px;">export</button>')
        var legendBox = $('<div class="routing_legend"></div>');
        legendBox.append('<div class="heading"><strong>Routing Modes<strong></div>');
        //create the table which holds the routing mode color descriptions
        var legendTable = $('<table class="legend_table"></table>');
        legendTable.append('<tr><td><canvas class="required" width="36" height="36"></canvas></td><td>Required</td></tr>');
        legendTable.append('<tr><td><canvas class="skippable" width="36" height="36"></canvas></td><td>Skippable</td></tr>');
        legendTable.append('<tr><td><canvas class="selectable" width="36" height="36"></canvas></td><td>Selectable</td></tr>');
        legendTable.append('<tr><td><canvas class="skippable_selectable" width="36" height="36"></canvas></td><td>Skippable and selectable</td></tr>');
        legendBox.append(legendTable);
        this.parent.append(legendBox);
        this.parent.append(download);
        this.setupEventHandlers();
        //draw the legend color blobs
        this.drawLegendCircle($('div.routing_graph canvas.required'), 'MODE_REQUIRED', 18, 18);
        this.drawLegendCircle($('div.routing_graph canvas.skippable'), 'MODE_SKIPPABLE', 18, 18);
        this.drawLegendCircle($('div.routing_graph canvas.selectable'), 'MODE_SELECTABLE', 18, 18);
        this.drawLegendCircle($('div.routing_graph canvas.skippable_selectable'), 'MODE_SKIPPABLE_SELECTABLE', 18, 18);
    }
}
...
emi2cRoutingGraph.prototype = {
    ...
    colors: {
        ...
        'MODE_SKIPPABLE': {'fill': '#f4f4f4', 'stroke': '#e0dcdc'},
