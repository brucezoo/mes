...
validatorToolBox={
    checkKey: function(name){
        ...
        !Validate.checkNewAttrKey(value)?(showInvalidMessage(name,"键值由3-50位字母下划线数字组成"),keyCorrect=!1,!1):
        (keyCorrect=1,!0);
    },
    checkName: function(name){
        ...
        Validate.checkNull(value)?(showInvalidMessage(name,"名称不能为空"),nameCorrect=!1,!1):
            Validate.checkNot0(value)?(showInvalidMessage(name,"名称不能为0"),nameCorrect=!1,!1):
        (nameCorrect=1,!0);
    },
    ...
    checkSelectData: function(name){
        ...
        if(ele.length){
            ...
            ele.each(function(index,item){
                var nval=$(item).val().trim();
                var numflag=0;
                allvalue.forEach(function(item){
                    if(item==nval){
                        numflag++;
                    }
                });
                if(Validate.checkNull(nval)){
                    selectCorrect=!1;
                    $('#dataTypeWrap').find('.errorMessage').html('选项不能为空').show();
                    return false;
                }else if(numflag>1){
                    selectCorrect=!1;
                    $('#dataTypeWrap').find('.errorMessage').html('选项值存在相同').show();
                    return false;
                } 
            });
        }else{
    ...
    remoteCheckKey: function(flag,name){
        ...
        getUnique(flag,name,value,function(rsp){
            if(rsp.results&&rsp.results.exist){
                keyCorrect=!1;
                var val='已注册';
                $('.el-button.el-button--primary.submit.add').addClass('is-disabled');
                showInvalidMessage(name,val);
            }else{
                keyCorrect=1;
                $('.el-button.el-button--primary.submit.add').removeClass('is-disabled');
            }
        });
    },
        ...
        success: function(rsp){
            ...
            if(rsp&&rsp.results){
                if(rsp.results.automatic_number=='1'){//自动生成允许手工改动
                    $('#addMAttr_from #key').val(rsp.results.code).removeAttr('readonly').attr('data-val',rsp.results.code);
                }else if(rsp.results.automatic_number=='2'){//自动生成不允许手工改动
                    $('#addMAttr_from #key').val(rsp.results.code).attr({'readonly':'readonly'});
                }else{
                    $('#addMAttr_from #key').removeAttr('readonly');
                }
            }else{
                console.log('获取物料属性键值失败');
            }
        },
        fail: function(rsp){
            // layer.close(layerLoading);
...
function addMaterielAttribute(data){
    AjaxClient.post({
        ...
        fail: function(rsp){
            ...
            $('#addMAttr_from .submit.add').removeClass('is-disabled');
        }
    },this);
}
//查看物料属性
...
function createNumHtml(data){
    ...
    var numHtml=`<div class="el-form-item">
        <div class="el-form-item-div">
            ...
            <input type="number" id="min_value" class="el-input" value="${min_value}" min="0">
        </div>        
    </div>
    <div class="el-form-item">
        <div class="el-form-item-div">
            <label class="el-form-item-label" style="width: 84px;">最大值</label>
            <input type="number" id="max_value" class="el-input" value="${max_value}" min="0">
        </div>
    </div>
    <div class="el-form-item">
        <div class="el-form-item-div">
            <label class="el-form-item-label" style="width: 84px;">默认值</label>
            <input type="number" id="default_value" class="el-input" value="${default_value}" min="0">
        </div>
...
$('body').on('keypress','#min_value',function(event){
    if(event.keyCode == 45){
        event.preventDefault();
    }
});
$('body').on('keypress','#max_value',function(event){
    if(event.keyCode == 45){
        event.preventDefault();
    }
});
$('body').on('keypress','#default_value',function(event){
    if(event.keyCode == 45){
        event.preventDefault();
    }
})
...
function createSelectItemHtml(num,item,selected){
    var itemDElete=`<div calss="el-item-wrap">
        ...
        itemAdd=`<div calss="el-item-wrap">
                <button type="button" class="el-button item-btn-add">+</button>
            </div>`;
    if(item){
        itemDElete='';
    }
                ...
                <div class="modal-wrap" style="max-height: ${height}">
                    <div class="el-form-item">
                        <div class="el-form-item-div">
                            ...
                            <input type="text" id="key" class="el-input" placeholder="由3-50位字母下划线数字组成" value="${key}">
                        </div>
...
function bindEvent(){
    ...
    $('body').on('click','#searchForm .submit:not(".is-disabled")',function(e){
        ...
        if(!$(this).hasClass('is-disabled')){
            ...
            ajaxData={
                ...
                label: encodeURIComponent(parentForm.find('#label').val().trim()),
    ...
    //添加选择项
    $('body').on('click','.formMateriel:not(".disabled") .item-btn-add',function(e){
        var ele=$(this).parents('.el-form-item.select-item');
        ele.after(createSelectItemHtml(1));
        $('#dataTypeWrap').find('.select-item').each(function(index,item){
...
$('#translate').on('click', function () {
    if($('#list').val() == 'null') {
        ...
        let a = '/Language/exportAttribute?_token = 8b5491b17a70e24107c89f37b1036078&languageCode=' + $('#list').val();
        $('#a').attr('href', a);
    }
})
