...
$(function () {
    ...
    $('body').on('click','.viewProcess',function (e) {
      ...
      var op_id = $(this).attr('data-operation-id');
      var wo_number = $(this).attr('data-wo-number');
      window.open("/BomManagement/bomSoView?sales_order_code="+so+"&sales_order_project_code="+spo+"&operation_id="+op_id+"&wo_number="+wo_number);
    });
...
function getRouting(id) {
    AjaxClient.get({
        url: URLS['thinPro'].routingShow + _token + "&work_order_id=" + id,
        dataType: 'json',
        beforeSend: function () {
            ...
            if(rsp.results.group_routing_package!=null){
                ...
                if(rsp.results.version_change==1){
                    ...
                    layer.confirm(msg, {
                        ...
                        layer.close(index);
                        window.open(`/BomManagement/bomFormView?id=${rsp.results.new_bom_id}`);
                    },function (index) {
                        layer.close(index);
        ...
        if (item.step_info && item.step_info.length) {
            item.step_info.forEach(function (sitem, sindex) {
                ...
                stepItems += `<tr>
                   ...
                   <td class="pre_bgcolor desc" style="min-width:80px;">${tansferNull(commentStr)}</td>
                 </tr>`;
                if(sitem.step_drawings && sitem.step_drawings.length){
                ...
                if(sitem.composing_drawings && sitem.composing_drawings.length){
                    sitem.composing_drawings.forEach(function (ditem,index) {
						...
						if (ditem.comment != '' && ditem.comment != null){
                            des+='描述：'+ditem.comment.replace("\n","");
                        }
                        step_draw+=`<div class="preview_draw_wrap" data-url="${ditem.image_path}">
                                     <p>
...
function createPreviewDoc(data,ability_name,attr,item_no,inspur_material_code,next_lifnr,sales_order_code,sales_order_project_code) {
    ...
    <tr><td style="padding-top: 10px;padding-left: 20px;-webkit-box-sizing: border-box;-moz-box-sizing: border-box;box-sizing: border-box;"><div id="qrcodewt"></div><div>${wo_num}</div></td><td colspan="2" style="text-align: center;font-size: 30px;font-weight: bold;">梦百合家居科技股份有限公司</td><td></td></tr>
                        ...
                        <tr style="font-size: 18px;"><td>销售订单：${sales_order_code}/${sales_order_project_code}</td><td>产品编码：${item_no}（${inspur_material_code}）</td><td>产品名称：${attr}</td><td>下道工序编号：${NEXT_LIFNR}</td></tr>
                    </table>
                    ${_mtable}
                    ${_thml}
                </div>
