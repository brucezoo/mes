...
(function($, window, document, undefined) {
                    ...
                    <table class="GanttTitleFixed GanttFixed">\
                        <thead>\
                            <tr>\
                                ...
                                <th><div style="width:70px;">剩余产能</th>\
                            </tr>\
                        </thead>\
                    </table>\
                </div>\
    ...
    function setDataList(orderList,options){
        for(var i=0;i<orderList.length;i++){
            ...
            var perTr=$('<tr data-work-bench-id="'+orderList[i].work_bench_id+'"><td style="height:'+tdHeight+'px"><span>'+orderList[i].name+'</span></td><td><p style="color:#119BE7;">'+(orderList[i].total_capacity-orderList[i].used_capacity).toFixed(0)+'s</p></td></tr>');
        ...
        if(dateObj.length){
            ...
            dateObj.forEach(function(ditem,i){
                ...
                tdele.append('<div class="abdiv" data-use-time="'+ditem.time+'" style="width: '+percent+'%;left: '+left+'px;bottom: ' +awayBottom+ 'px;"><span class="Tname" taskid=""><i></i></span>\
                    ...
                    <div class="pro"><span class="finished" style="border-radius:5px;cursor:pointer;height:20px;text-align:left;width:100%;color:#333;background:  '+rgb()+'">'+ditem.number+'</span></div></div>');
