$.fn.bindUpload = function () {
    var myData = {
        ...
        owner:arguments[0].owner||"abnormal"
    }
    var _this = this;
    ...
    this.upload = function (max, i, item) {
        ...
        formData.append("owner", myData.owner);
        formData.append("_token", TOKEN);
        if(myData.owner=='complaint'){
          formData.append("attachment", item);
          formData.append("flag", myData.owner);
          formData.append("file_id",0);
          formData.append('creator_token','24ed0q9gukcdnnb3tjcna1unj7');
        }
        $.ajax({
            url: myData.url,
            type: "post",
                ...
                if (data.code == 200) {
                    ...
                    if(myData.owner=='complaint'){
                      callInput.find('.listBox').append('<li>' +
                        '<img data-id="'+data.results.attachment_id+'"  src="/storage/' + data.results.path + '"/>' +
                        '</li>');
                    }else{
                      callInput.find('.listBox').append('<li>' +
                        '<img data-id="'+data.results.insert_id+'"  src="/storage/' + data.results.image_path + '"/>' +
                        '<span data-id="'+data.results.insert_id+'" data-path="'+data.results.image_path+'" class="delete"></span>' +
                        '</li>');
                    }
                    ...
                    _this.find("#zwb_upload_status").fadeOut(100, function () {
                        $(_this).find("#zwb_upload_status").remove();
                    });
                } else {
