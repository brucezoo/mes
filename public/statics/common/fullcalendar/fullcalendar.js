...
var DayGrid = /** @class */ (function (_super) {
    ...
    DayGrid.prototype.renderNumberCellHtml = function (date) {
        ...
        if (isDayNumberVisible) {
            ...
            // dateInfo=HolidayChina(newDate).getDateInfo(newDate),
            // holidayList = HolidayChina(newDate).getHolidayDateList(dateInfo.yearMonth),
            // modList = HolidayChina(newDate).getMod();
            // 增加节气显示
            var cTerm = lunar(newDate).term;
            if (cTerm) {
                html += "<div class='fc-day-cnTerm'>" + cTerm + "</div>";
            }
            // 增加节日显示
            var fes = lunar(newDate).festival();
            if (fes && fes.length > 0) {
                html += "<div class='fc-day-cnTerm'>" + $.trim(fes[0].desc) + "</div>";
            }
            // 无节气和节日时显示农历
            if (!cTerm && (!fes || fes.length == 0)) {
                html += "<div class='fc-day-cnTerm'>" + lunar(newDate).lDate + "</div>";
            }
            //增加休息日
            // if("1" == holidayList[dateInfo.dateStr]){
            //      html += "<div class='fc-day-holiday-sign'>休</div>";
            // }else if("2" == holidayList[dateInfo.dateStr]){
            //     html += "<div class='fc-day-holiday-sign'>班</div>";
            // }
        }
        html += '</td>';
        return html;
