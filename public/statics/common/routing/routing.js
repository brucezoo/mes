 ...
 */
var rnodes=null,controlCode={}
current_click_routing_node_id=0,
        current_click_operation_id=0;

function actrnodes() {
  var rinfo=[];
  var preele=$('.make_list_all .make-item.selected');
  if(!preele.length){
    // preele=$('.self-make-btn.selected');
    preele=$('.self-make-btn');
  }
  var errorflag=true;
  if(preele.length){
    $('.route-tbody tr.step-tr').each(function (trindex,tritem) {
      var is_start_or_end=1,step_path=[],startObj={},groupIndex='';
      if($(tritem).attr('data-group-id')){
        var groupid=$(tritem).attr('data-group-id');
        groupIndex=groupid;
        $('tr.step-tr[data-group-id='+groupid+']').each(function (index,sitem) {
          step_path.push($(sitem).attr('data-field-id'));
        });
        if($(tritem).find('.s-e-check.start.is-checked').length){
          is_start_or_end=2;
          startObj={
            start_step_path: groupid.split('-')[0].split('_')[0],
            index: groupid.split('-')[0].split('_')[1]
          };
        }else if($(tritem).find('.s-e-check.end.is-checked').length){
          is_start_or_end=3;
          startObj={
            start_step_path: groupid.split('-')[0].split('_')[0],
            index: groupid.split('-')[0].split('_')[1]
          ...
          is_start_or_end=1;
          startObj={
            start_step_path: $(tritem).attr('data-field-id'),
            index: $(tritem).attr('data-index')
          };
        }
      }else{
        var groupid=$(tritem).attr('data-step-id');
        groupIndex=groupid+'-'+groupid;
        step_path.push($(tritem).attr('data-field-id'));
        startObj={
          start_step_path: $(tritem).attr('data-field-id'),
          index: $(tritem).attr('data-index')
        };
      ...
      var trobj={
        // factory_id:$('#route_id').attr('data-factoryId'),
        routing_node_id: preele.attr('data-node-id'),
        practice_id: preele.attr('data-id'),
        operation_id: preele.attr('data-opid'),
        material_category_id: $(tritem).attr('data-cate-id')||'-1',
        step_id: $(tritem).attr('data-field-id'),
        field_description: $(tritem).attr('data-field-desc'),
        practice_step_order_id: $(tritem).attr('data-pfid')||'0',
        index: $(tritem).attr('data-index'),
        field_id: $(tritem).attr('data-field-id'),
        name: $(tritem).attr('data-name'),
        workcenters:[],
        code: $(tritem).attr('data-code'),
        stand_values:JSON.parse($(tritem).attr('data-standValues')),
        workHours:JSON.parse($(tritem).attr('data-workhours')),
        select_type: 0,
        group_index: groupIndex,
        is_start_or_end: is_start_or_end,
        is_template: $(tritem).attr('data-isTemplate')=='undefined'?0:$(tritem).attr('data-isTemplate'),
        material_info: [],
        practice_work_hour: 0,
        // description: $(tritem).find('.basic-text').text()==null?'':$(tritem).find('.basic-text').text(),
        description: $(tritem).find('.basic-text').text(),
        comment: $(tritem).find('.ma-textarea').val().trim(),
      ...
      if($(tritem).find('.mode-wrap .mode-check.is-checked').length){
        trobj.select_type=$(tritem).find('.mode-wrap .mode-check.is-checked').index()+1;
        // console.log(trobj.select_type);
      }
      //能力带出工作中心id
      //   $(tritem).find('.el-form-item.ma-ability_wrap').each(function(i,val){
      //     var datajson=JSON.parse($(val).attr('data-json'));
      //     trobj.ability_info.push(datajson);
      //   })

      //步骤带出工作中心id
        var workcenters = [];
        $(tritem).find('.el-form-item.ma-workcenters_wrap').each(function(i,val){
            if ($(val).attr('data-json')){
                var datajson=JSON.parse($(val).attr('data-json'));
                // console.log(datajson);
                datajson.forEach(function(item){
                    workcenters.push(item.workcenter_id);
                });
                trobj.workcenters=workcenters;
            }
        })

        if($(tritem).find('.el-form-item.ma-device_wrap .device-val').length) {
          if($(tritem).find('.el-form-item.ma-device_wrap .device-val').val() == 'undefined') {
            trobj.device_id = '0';
          } else {
            trobj.device_id =  $(tritem).find('.el-form-item.ma-device_wrap .device-val').val() || '0';
          }
        } else {
          trobj.device_id = '0';
        }
      ...
      //校验能力
      if($(tritem).find('.el-form-item.ma-ability_wrap').length){
        var absele=$(tritem).find('.el-form-item.ma-ability_wrap'),
          abs=absele.find('.val_id').val().trim();
        if(!abs){
          if($(tritem).find('.s-e-check.end.is-checked').length||$(tritem).find('tr.outcome').length){
            LayerConfig('fail','请检查工艺路线进料数量及出料能力选择');
      ...
      //校验物料数量
      $(tritem).find('.route_item tbody tr').each(function (iindex,iitem) {
        if (!$(iitem).hasClass('name-attr-show')) {

            if($(iitem).find('.el-input.el-input-num').val().trim()==''){
              LayerConfig('fail','请检查工艺路线进/出料数量及出料能力选择');
              errorflag=false;
              return false;
            }
            // console.log($(iitem).attr('data-json'));
            var datajson=JSON.parse($(iitem).attr('data-json'));
            let userHandNum = '';
            let userNum = '';
            if($(iitem).attr('data-is-lzp') !== '0') {
              if(($(iitem).find('.el-input.el-input-num').val().trim() == '0') || ($(iitem).find('.el-input.el-hand-num').val().trim() == '0')) {
                LayerConfig('fail','请检查工艺路线进/出料数量');
                $('.save-route').removeClass('preventRedraw');
                errorflag=false;
                return false;
              }
              userHandNum = $(iitem).find('.el-input.el-hand-num').val().trim();
              userNum = $(iitem).find('.el-input.el-input-num').val().trim();
              if (datajson.commercial == 'PC') {
                userNum = Math.ceil(Number(userNum));
              }
            } else {
              if(($(iitem).find('.el-input.el-input-num').val().trim() == '0')) {
                LayerConfig('fail','请检查工艺路线进/出料数量');
                $('.save-route').removeClass('preventRedraw');
                errorflag=false;
                return false;
              ...
              userHandNum = $(iitem).find('.el-input.el-input-num').val().trim();
              userNum = $(iitem).find('.el-input.el-input-num').val().trim();
            }

            var maObj=datajson;
            maObj.step_path=step_path.join(',');
            maObj.index=iindex+1;
            maObj.is_lzp=$(iitem).attr('data-is-lzp');
            maObj.type=$(iitem).attr('data-type');
            maObj.use_number=userNum;
            maObj.use_num=userNum;
            maObj.user_hand_num=userHandNum;
            maObj.desc=$(iitem).find('.describe').val().trim();
            material_info.push(maObj);
        }
      });
      var drawings=[];
      if($(tritem).attr('data-imgs')){
        var sdrawings=JSON.parse($(tritem).attr('data-imgs'));
        sdrawings.forEach(function (ditem, index) {
          var dobj={
            sort: index+1,
            ...
            comment: '',
            compoing_drawing_id: ditem.compoing_drawing_id || 0,
            is_show: ditem.is_show || 1
          };
          drawings.push(dobj);
        });
      }

      trobj.drawings=drawings;
      if($(tritem).attr('data-attch')){
        trobj.attachments=JSON.parse($(tritem).attr('data-attch'));
      }
      trobj.material_info=material_info;
      trobj.operation_ability_ids=abs;
      rinfo.push(trobj);
    });
    ...
    setTimeout(function () {
        rnodes.ipractice_id=preele.attr('data-id');
    }, 20);
    rnodes.iroutes=[];
    rnodes.iroutes=rinfo;
  }
  return errorflag;
}
...
// 根据工序获取工作中心
function getWorkcenters(opnode) {
  var stepsId = [],factoryId  = $('#route_id').attr('data-factoryId');
  // if(opnode.iroutes){
  //     opnode.iroutes.forEach(function(val,i){
  //         stepsId.push(val.step_id);
  //     })
  // }
  var stepIds = JSON.stringify(stepsId);
  $.ajax({
      type:"get",
      url: URLS['bomAdd'].getWorkcenters + '?' +_token,
      async:false,
      dataType: 'json',
      data : {
        operation_id : opnode.operation_id,
        step_ids : stepIds,
      ...
      success:function (rsp) {
          ...
          if(rsp&&rsp.results){
              // workCenters={};
              workCenters=rsp.results;
          }
      },
      error:function (rsp) {
          layer.close(layerLoading);
          console.log('获取工作中心失败');
      }
  })
}

function RoutingGraph(parentElementName, routingGraph, width, height) {
  this.width = width;
  this.height = height;
  this.margin = 64;
  ...
  if (this.parent.length) {
    ...
    var download=$('<button class="btn copy-btn" style="position: absolute;left: 10px;top: 10px; padding:4px;">复制工艺</button>')
...
RoutingGraph.prototype = {
  ...
  ajaxFn: function (node) {
    ...
    $('.self-make-btn').attr({
      'data-opcode': node.operation_code,
      'data-node-id': node.oid,
      'data-id': '-1',
      'data-opid': node.operation_id
    });
    AjaxClient.get({
          ...
          if (clickNode != that.clickNode) {
            if (clickNode) {
              ...
              var changeFlag = false;
                ...
                current_click_routing_node_id=that.clickNode.oid,
                    current_click_operation_id=that.clickNode.operation_id,
                choose_code.forEach(function (item) {
                    if(item.routing_node_id==that.clickNode.oid){
                      $("#select_type_code .el-select-dropdown-item[data-id="+item.control_code+"]").click();
                        changeFlag=true;
                    }
                });
                if(!changeFlag){
                    $("#select_type_code .el-select-dropdown-item[data-id='PP01']").click();
                }

              getWorkcenters(that.clickNode);
              $('.make_list_line_all').html('');

              if($('.line_container .line-check.is-checked').length){//有选中的做法线

                // $('.line_container .line-check.is-checked').each(function (index, item) {
                  var makeJSON=$('.make_list_line_all .line-check.is-checked').parents('.line_container').attr('data-line'),
                  makeObj=JSON.parse(makeJSON);
                  
                  if(makeObj.operation&&makeObj.operation.length){
                    var oppractices=[];
                    makeObj.operation.forEach(function (opitem) {
                      if(opitem.operation_id==that.clickNode.operation_id){
                        oppractices=opitem.practice;
                        return false;
                      }
                    });
                   
                    if(oppractices.length){
                      var ele=$('.make_list_all');
                      ele.html('');
                      $('.route_overflow .route-tbody').html('<tr><td colspan="11" style="text-align: center;">暂无数据</td></tr>');
                      rnodes=null;
                      rnodes=that.clickNode;
                      var listHtml='';
                      oppractices.forEach(function (mitem) {
                        listHtml+='<div class="make-item" data-opcode="'+rnodes.operation_code+'" data-node-id="'+rnodes.oid+'" data-id="'+mitem.practice_id+'" data-opid="'+rnodes.operation_id+'">'+mitem.practice_names+'</div>';
                      });
                      ele.html(listHtml);
                      setTimeout(function () {
                        if(rnodes.ipractice_id){
